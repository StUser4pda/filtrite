...
func GenerateDistributableList(inputPaths []string, output string, logPath string) (err error) {
	...
	cmd := exec.Command(rulesetExecutable, "--chrome_version=54", "--input_format=filter-list", "--output_format=unindexed-ruleset", "--input_files="+strings.Join(inputPaths, ","), "--output_file="+output)
